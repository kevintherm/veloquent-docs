...
<head>
    ...
    <meta name="description"
        content="{{ $description ?? 'Veloquent is an open-source Backend as a Service (BaaS) built on top of Laravel, simplifying the development of modern web and mobile applications.' }}">
    <meta name="keywords"
        content="Laravel, BaaS, Backend as a Service, PHP, Open Source, Realtime Database, Authentication, API Rules">
...
<body class="p-4 sm:p-8 md:p-12">
    ...
    <nav class="max-w-350 mx-auto flex flex-col sm:flex-row justify-between items-center gap-8 mb-16 sm:mb-24">
        <a href="/"
            class="p-4 sm:p-5 brutalist-border bg-white brutalist-shadow transition-transform hover:-translate-y-1">
        ...
        <div class="flex flex-wrap justify-center gap-4 sm:gap-8">
            <a href="/docs" class="brutalist-button text-lg sm:text-xl">DOCS</a>
            ...
            <div class="grid grid-cols-1 md:grid-cols-10 gap-16 mb-24">
                <div class="md:col-span-4">
                    ...
                    <div class="flex gap-4">
                        <a href="https://github.com/kevintherm/veloquent"
                            class="p-3 brutalist-border hover:bg-black hover:text-white transition-colors">
                ...
                <div class="md:col-span-2">
                    ...
                    <ul class="flex flex-col gap-4 mono font-bold uppercase text-lg">
                        @php
                            $docsManager = app(\App\Docs\DocsManager::class);
                            $latestVersion = $docsManager->getLatestStableVersion();
                            $footerDocs = collect();
                            if ($latestVersion) {
                                $sidebar = $docsManager->getSidebar($latestVersion);
                                foreach ($sidebar as $category) {
                                    foreach ($category['files'] as $file) {
                                        $footerDocs->push($file);
                                        if ($footerDocs->count() >= 3) {
                                            break 2;
                                        }
                                    }
                                }
                        ...
                        @foreach ($footerDocs as $doc)
                            <li><a href="{{ route('docs.show', ['version' => $latestVersion, 'file' => $doc['path']]) }}"
                                    class="hover:underline">{{ $doc['title'] }}</a></li>
                        @endforeach
                    </ul>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <ul class="flex flex-col gap-4 mono font-bold uppercase text-lg">
                        ...
                        <li><a href="https://github.com/kevintherm/veloquent/discussions" target="_blank"
                                class="hover:underline">Discussions</a></li>
                        <li><a href="mailto:support@velophp.com" class="hover:underline">Contact</a></li>
                    </ul>
                </div>
            </div>
