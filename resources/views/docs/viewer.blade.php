...
<head>
    ...
    @endisset
    <meta name="keywords"
        content="Laravel, BaaS, Backend as a Service, PHP, Open Source, Realtime Database, Authentication, {{ $title ?? '' }}">
    ...
    <meta property="og:description"
        content="{{ $description ?? 'Veloquent is an open-source Backend as a Service (BaaS) built on top of Laravel.' }}">
    ...
    <style>
        ...
        .content-area h1 .heading-permalink {
            left: -4rem;
        }
...
<body class="min-h-screen flex flex-col md:flex-row scroll-smooth">
    ...
    <div class="md:hidden p-6 bg-white border-b-8 border-black sticky top-0 z-50 flex justify-between items-center">
        ...
        <button id="menu-toggle"
            class="px-6 py-3 bg-blue-500 text-white font-black border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all">MENU</button>
    </div>

    <!-- Sidebar -->
    <aside id="sidebar"
        class="hidden md:block w-full md:w-96 border-r-4 border-black/5 shrink-0 sticky top-0 h-screen overflow-y-auto z-40">
        ...
        <div class="p-8 border-b border-black/5 bg-white">
            <form action="{{ route('docs.search') }}" method="GET" class="relative">
                <input type="text" name="q" value="{{ $search_query ?? '' }}" placeholder="SEARCH DOCS..."
                    class="w-full px-6 py-4 bg-slate-50 border-2 border-black/10 text-black font-black  tracking-tighter placeholder-black/20 focus:border-blue-500 focus:bg-white transition-all outline-none">
                @if(isset($version))
                    <input type="hidden" name="version" value="{{ $version }}">
                @endif
                <button type="submit"
                    class="absolute right-4 top-1/2 -translate-y-1/2 font-black text-black/30 hover:text-blue-600 transition-colors">↵</button>
            </form>
        </div>
                ...
                @foreach($sidebarCategories as $categoryKey => $category)
                    ...
                    <div class="flex flex-col">
                        @foreach($category['files'] as $doc)
                            ...
                            <a href="/{{ config('docs.path', 'docs') }}/{{ $version ?? 'unknown' }}/{{ $doc['path'] }}"
        ...
        <div class="max-w-4xl mx-auto">
            <div class="mb-20 flex items-center gap-8 justify-between">
                <div class="flex items-center gap-8 flex-1">
                    <div class="bg-blue-500 text-white px-6 py-3 font-black  tracking-widest border-4 border-black">
                        {{ config('velo.version', '1.0.0') }} DOC_TYPE:
                        {{ isset($search_results) ? 'SEARCH_QUERY' : 'SYSTEM_REFERENCE' }}
                    </div>
                    <div class="h-2 bg-black grow"></div>
                </div>
                @if(isset($version))
                    <x-version-switcher :availableVersions="$availableVersions" :version="$version"
                        :headingAnchor="$headingAnchor ?? ''" :currentPath="$currentPath ?? ''" />
                @endif
            </div>
            ...
            <div class="content-area">
                @if(isset($search_results))
                    ...
                    <p class="mb-12 font-black text-2xl tracking-tighter">{{ count($search_results) }} matches found in core
                        system docs</p>

                    <div class="space-y-8">
                        @foreach($search_results as $result)
                            <div class="bg-white p-10 border-8 border-black shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] -rotate-1 hover:rotate-0 transition-transform cursor-pointer"
                                onclick="window.location='/{{ config('docs.path', 'docs') }}/{{ $result['version'] }}/{{ $result['path'] }}'">
                                <h2 class="text-3xl font-black mb-4 italic !mt-0 !border-0 !p-0 underline">
                                    {{ $result['title'] }}
                                </h2>
                                <p class="text-xl font-medium mb-6 leading-relaxed">{!! $result['snippet'] !!}</p>
                                <a href="/{{ config('docs.path', 'docs') }}/{{ $result['version'] }}/{{ $result['path'] }}"
                                    class="text-blue-600 font-black  tracking-widest border-b-4 border-blue-600 hover:bg-blue-600 hover:text-white transition-all">View
                                    Document →</a>
                            </div>
                        @endforeach
                    </div>
            ...
            <!-- Footer -->
            <footer
                class="mt-40 pt-20 border-t-8 border-black flex flex-col md:flex-row justify-between items-start md:items-center gap-12">
                <div>
                    <img src="/logo.svg" alt="Velo" class="h-8 mb-4">
                    <p class="font-black  tracking-tighter">Open Source Backend</p>
                    @if(isset($version))
                        <p class="text-sm font-bold text-black/50 mt-2">Viewing docs for version:
                            {{ app(\App\Docs\DocsManager::class)->getVersionLabel($version) }}
                        </p>
                    @endif
                </div>
                <div class="flex gap-8">
                    <a href="https://github.com/kevintherm/veloquent" target="_blank"
                        class="bg-black text-white px-8 py-4 font-black border-4 border-black hover:bg-white hover:text-black transition-all">GITHUB</a>
                    <a href="https://github.com/kevintherm/veloquent/blob/main/TODO.md" target="_blank"
                        class="bg-blue-500 text-white px-8 py-4 font-black border-4 border-black hover:bg-black transition-all">ROADMAP</a>
                </div>
            </footer>
        </div>
    </main>
